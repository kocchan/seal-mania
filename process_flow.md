...
## システム概要
このシステムは、特定キーワード（ボンボンドロップ、ぷっくりシール）に関連するツイートをYahoo!リアルタイム検索から自動収集し、フィルタリングして保存するシステムです。
...
graph TD
    ...
    subgraph "初期化フェーズ"
        B --> B1[queries.json<br/>検索クエリ読み込み]
        B --> B2[ng_words.json<br/>NGワード設定読み込み]
        B --> B3[official_accounts.json<br/>公式アカウント一覧読み込み]
        B --> B4[blacklist.txt<br/>ブラックリスト読み込み]
        B --> B5[tweets.json<br/>既存ツイートデータ読み込み]
    end
    ...
    subgraph "スクレイピングフェーズ"
        C --> C1[Playwright起動<br/>Chrome headlessモード]
        C1 --> C2[検索クエリごとにループ]
        C2 --> C3[Yahoo!検索URL構築]
        C3 --> C4[ページアクセス & 待機]
        C4 --> C5[DOM要素検索<br/>複数セレクタで試行]
        C5 --> C6[ツイートデータ抽出]
        C6 --> C7{他のクエリあり？}
        C7 -->|Yes| C2
        C7 -->|No| D
    ...
    D[📝 取得データ統合] --> E[🔄 フィルタリング開始]
    
    subgraph "フィルタリングフェーズ"
        E --> F1{公式アカウント？}
        F1 -->|No| DISCARD1[❌ 破棄]
        F1 -->|Yes| F2{ブラックリスト？}
        ...
        F3 -->|重複| DISCARD3[❌ 破棄]
        F3 -->|新規| F4{テキスト重複チェック<br/>先頭100文字で判定}
        
        F4 -->|重複| DISCARD4[❌ 破棄]
        F4 -->|新規| F5{URL存在チェック}
        
        F5 -->|URLなし| DISCARD5[❌ 破棄]
        F5 -->|URLあり| F6{NGワード/スパムチェック}
        
        F6 -->|検知| BAN[🚫 自動BAN実行]
        BAN --> BAN1[ブラックリストに追加]
        BAN1 --> DISCARD6[❌ 破棄]
        
        F6 -->|クリーン| KEEP[✅ 保存対象として採用]
    end
